        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold hidden sm:inline">Compare Products</span>
              <Badge variant="secondary">{compareProducts.length}/{PRODUCT_LIMITS.MAX_COMPARISON}</Badge>
            </div>
            <Button variant="ghost" size="sm" onClick={clearCompare} className="text-muted-foreground hover:text-destructive">
          ...
          <div className="flex items-center gap-2 overflow-x-auto max-w-[50%] px-2">
            {compareProducts.map((product) => (
              ...
              >
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-md border bg-muted overflow-hidden">
                  <Image
            ...
            {Array.from({ length: PRODUCT_LIMITS.MAX_COMPARISON - compareProducts.length }).map((_, i) => (
              <div 
                key={`empty-${i}`}
                className="w-14 h-14 sm:w-16 sm:h-16 rounded-md border-2 border-dashed border-muted-foreground/30 flex items-center justify-center flex-shrink-0"
              >
